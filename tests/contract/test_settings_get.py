"""Contract test for GET /api/plugins/subway_scaler/settings."""
...
def test_get_returns_defaults_when_missing(client, tmp_settings_path):
    ...
    r = client.get("/api/plugins/subway_scaler/settings")
...
def test_get_returns_stored_after_put(client):
    ...
    r = client.put("/api/plugins/subway_scaler/settings", json=new)
    assert r.status_code == 200, r.text
    r2 = client.get("/api/plugins/subway_scaler/settings")
