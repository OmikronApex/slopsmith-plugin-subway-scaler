...
- 2026-05-28: Variant-track lamppost fix (two parts). (1) Commit timing: the riding/accept flow has no dismiss piece, so `_commitBuildingTransition()` was only reached via the next propose's `clearVariantGeom()` (the `_charTraversal`-completion call fires before `spawnVariantTracks()` sets `_worldOffsetX`, so it early-returns). Added `_commitBuildingTransition()` at the top of `finalizeVariantTransition()` (main.js `applyPromoteResponse`, at landing, where old tracks are dropped) so the env swap lands on arrival. Idempotent — existing call sites unaffected. (2) Variant-track coverage: light-less preview lamp pool (`createVariantLamppostPool`/`clearVariantLamppostPool`, `variantLeft/RightLampposts`, `_variantLampOffsetX`; `makeLamppostGroup(side, withLight=false)` omits the SpotLight to avoid a mid-game light-count change → shader recompile / lag spike) spawned in `proposeVariantTracks()` at the variant offset and scrolled in the render loop, so the new track shows lampposts during the decision/ride window. At landing `_commitBuildingTransition()` reparents the old main pool's SpotLights onto these exact preview lamps (light count constant → no recompile, no positional pop) and adopts them as the new main pool; old lamps disposed light-less. Cleared on dismiss in `clearVariantGeom()`/`reset()`; shared materials preserved on mid-game clear. 82/82 pytest pass, JS syntax check clean.
